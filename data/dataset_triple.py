import torch
import os
import torchvision.transforms as transforms
import cv2
import numpy as np
import random


def print_dict(data):
    for name in data:
        print(f'{name} : {data[name]}')


class DatasetTools(object):

    def get_video_datainfo(self, data_root, balance = True) -> list:
        
        data_infos = []
        class_counts = {}
        cls_folders = os.listdir(data_root)  
        cls_folders = [x for x in cls_folders if x != 'none'] 
        cls_folders.sort() # [0, 1, 2, 3]
        

        for i in range(4):           
            class_counts[str(i)] = 0 # class_counts = {'0': 0, '1': 0, '2': 0, '3': 0}

        
        for i, folder in enumerate(cls_folders): 

            if i == 0:
                label = 0
            elif i == 1:
                label = 1
            elif i == 2:
                label = 2
            elif i == 3:
                label = 3


            videos = os.listdir(data_root + '/' + folder + '/') 
            for video in videos:
                class_counts[str(label)] += 1

        
        max_n = max([class_counts[name] for name in class_counts]) 

        for i, folder in enumerate(cls_folders):

            if i == 0:
                label = 0
            elif i == 1:
                label = 1
            elif i == 2:
                label = 2
            elif i == 3:
                label = 3


            videos = os.listdir(data_root + '/' + folder + '/') 
            for video in videos:
                tmp = {
                    'video_path': data_root + '/' + folder + '/' + video + '/',  
                    'label': label
                }
                if balance:
                    repeat_time = int(max_n / max(class_counts[str(label)], 1)) 
                else:
                    repeat_time = 1
                for _ in range(repeat_time):
                    data_infos.append(tmp) 

        return data_infos, class_counts


    def build_sequence_datainfo(self, 
                                video_data_infos: list, 
                                clip_length: int, 
                                sequence_overlap: int) -> list: 
                        
        seq_data_infos = []
        class_counts = {} # just for analysis
        for video_info in video_data_infos: # {video_path, label}
            
            tmp = {'imgs': [], 'label': video_info['label']}
            if video_info['label'] not in class_counts:
                class_counts[tmp['label']] = 0

            n_frames = len(os.listdir(video_info['video_path'])) 

            if n_frames < clip_length: 
                continue

            else:
                max_start = n_frames - clip_length  
                start_pt = list(range(0, max_start + 1, clip_length))
                
                for pt in start_pt:

                    tmp = {'imgs': [], 'label': video_info['label']}

                    for i in range(pt, pt + clip_length):  
                        tmp['imgs'].append(video_info['video_path'] + f'{i}.jpg')

                    seq_data_infos.append(tmp)  
                    class_counts[tmp['label']] += 1 
   
        return seq_data_infos, class_counts


    def build_clip(self, data_info, data_size, trans):
        clip = []

        for img_path in data_info['imgs']:
            arr = cv2.imread(img_path) 
            arr = arr.astype(np.float32) 
            t = torch.from_numpy(arr) 
            t = t / 255.0 
            t = t.permute(2, 0, 1) # H, W, C -> C, H, W
        
            # resize
            target_size = [112, 112]
            resize_func = transforms.Resize(target_size, antialias=True)
            t = resize_func(t)

            clip.append(t)
            
        clip = torch.stack(clip) #[num_frames, C, H, W]
        clip = trans(clip)
        
        return clip


class VideoDataset(torch.utils.data.Dataset):

    def __init__(self, 
                 balance: bool,
                 trans: bool,
                 data_root: str, 
                 clip_length : int = 8, 
                 clip_select_range: int = 12, 
                 data_size: tuple = [112, 112], # [H, W]
                 sequence_overlap: int = 4): 


        assert data_size[0] == data_size[1] # So for, only support square frames
        self.data_size = data_size
        self.clip_length = clip_length
        self.clip_select_range = clip_select_range
        
        self.dtools = DatasetTools()

        self.trans = trans
        if trans:
            self.transforms = transforms.Compose([
                    transforms.ColorJitter(0.08, 0.08, 0.08, 0.02), # brightness, contrast, saturation, hue
                    transforms.RandomHorizontalFlip(),
                    transforms.RandomGrayscale(),
                    transforms.Normalize(0.5, 0.5),
                ])
        else:
            self.transforms = transforms.Compose([
                    transforms.Normalize(0.5, 0.5),
                ])

        self.balance = balance
        if balance:
            balance = True
        else:
            balance = False

        self.video_data_infos, class_counts = self.dtools.get_video_datainfo(data_root, balance)

        self.seq_data_infos, class_counts = \
            self.dtools.build_sequence_datainfo(self.video_data_infos, clip_length, sequence_overlap)

        print_dict(class_counts)


    def __len__(self):
        return len(self.seq_data_infos)

        
    def __getitem__(self, index):

        anchor_info = self.seq_data_infos[index]
        anchor_data = self.dtools.build_clip(anchor_info, self.data_size, self.transforms)
        anchor_label = anchor_info['label']

        # positive
        positive_info = self.get_positive(anchor_label, index)
        positive_data = self.dtools.build_clip(positive_info, self.data_size, self.transforms)

        # negative
        negative_info = self.get_negative(anchor_label)
        negative_data = self.dtools.build_clip(negative_info, self.data_size, self.transforms)

        anchor_data = anchor_data.transpose(0, 1)  # [T, C, H, W] -> [C, T, H, W]
        positive_data = positive_data.transpose(0, 1)
        negative_data = negative_data.transpose(0, 1)

        return anchor_data, positive_data, negative_data, anchor_label 



    def get_positive(self, label, index):
        same_class_indices = [i for i, info in enumerate(self.seq_data_infos) if info['label'] == label and i != index]
        positive_index = random.choice(same_class_indices) 
        return self.seq_data_infos[positive_index]

    def get_negative(self, label):
        different_class_indices = [i for i, info in enumerate(self.seq_data_infos) if info['label'] != label]
        negative_index = random.choice(different_class_indices)
        return self.seq_data_infos[negative_index]


        


